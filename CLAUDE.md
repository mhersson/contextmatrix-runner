...
GitHub authentication is not an internal package: the runner imports the shared
`github.com/mhersson/contextmatrix-githubauth` module from
...
- **`github.com/mhersson/contextmatrix-githubauth`** — shared GitHub auth module
  (App + PAT + caching). GitHub App JWT signing lives inside this module;
...
- Worker containers run `claude --allowed-tools` with an explicit allowlist
  instead of `--dangerously-skip-permissions`. See the `ALLOWED_TOOLS_COMMON`
  and `ALLOWED_TOOLS_AUTO_EXTRAS` arrays in `docker/entrypoint.sh`. HITL mode
  uses `COMMON` only; autonomous mode appends `Task` so sub-agents can spawn.
- `mcp__contextmatrix__chat_rehydration_complete` lives in
  `ALLOWED_TOOLS_COMMON`. Chat mode resolves
  `ALLOWED_TOOLS_CHAT = COMMON + AUTO_EXTRAS`, so the tool is callable from
  rehydrating agents. Removing or renaming it silently breaks the rehydration
  flow — the agent gets blocked by the permission gate and the "Restoring
  workspace…" banner in the UI never lifts.
- `LSP` (Claude Code's built-in LSP gateway) is in `ALLOWED_TOOLS_COMMON`. All
  LSP plugins (`gopls-lsp@claude-plugins-official`,
  `typescript-lsp@claude-plugins-official`) route through this single tool. The
  plugins are enabled per-container via `claude_settings`'s `enabledPlugins`
  field. The worker image ships `gopls` and `typescript-language-server`
  binaries; to add a new language, install its LSP binary in `Dockerfile.worker`
  and add the plugin name to `enabledPlugins`.
...
`github.com/mhersson/contextmatrix-protocol` module, imported by both the runner
and ContextMatrix, so the two sides cannot drift. Both sides sign as
...
and URI into the signature prevents a valid signature for one endpoint from
being replayed against another, and prevents two same-second GETs to the same
path with different query strings from colliding in the replay cache. Headers:
...
`internal/webhook/middleware.go`). The same value is echoed on every response —
including error responses and SSE streams — so CM and external tools can stitch
their traces to runner logs.
...
| POST   | `/message`     | HMAC | Send a user message to a running session. Card-mode payload: `{card_id, project, content, message_id}`. Chat-mode payload: `{session_id, content, message_id}` (mutually exclusive with `card_id`/`project`). Always written to the container's stdin as a plain user-text stream-json frame. `content` must be ≤8192 bytes (413 on overflow). Returns 404 if no container, 409 if not interactive, 202 `{ok:true, message_id}` on success. AskUserQuestion routing is handled at the CM MCP permission gate (`mcp__contextmatrix__permission_prompt`), not through this webhook.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
- Operators interact with the running session via:
  - `POST /message` — writes a plain user-text stream-json frame to the
    container stdin. Echoes the content as a `user`-typed `LogEntry`.
...
all SDK calls flow through the `container.Manager` allowlist + bounded contexts.
Re-run `govulncheck ./...` after every Moby SDK upgrade and drop this note once
