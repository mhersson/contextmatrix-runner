...
| POST   | `/message`     | HMAC | Send a user message to a running session. Card-mode payload: `{card_id, project, content, message_id}`. Chat-mode payload: `{session_id, content, message_id}` (mutually exclusive with `card_id`/`project`). Both payloads accept an optional `tool_use_id` — when non-empty, the runner writes a stream-json `tool_result` frame referencing that id instead of the default user-text frame, used to close an open `AskUserQuestion` tool call. Empty string or omitted preserves the plain user-text behavior. `content` must be ≤8192 bytes (413 on overflow). Returns 404 if no container, 409 if not interactive, 202 `{ok:true, message_id}` on success. |
...
- Operators interact with the running session via:
  - `POST /message` — writes a stream-json frame to the container stdin
    (user-text by default, or a `tool_result` frame when the payload's
    optional `tool_use_id` is non-empty, to close an `AskUserQuestion`
    tool call). Echoes the content as a `user`-typed `LogEntry`.
