...
// dnsCache is a TTL+capacity-bounded LRU of DNS resolutions, modelled on
// webhook.ReplayCache. Thread-safe. A zero-value cache is NOT usable — use
// newDNSCache to construct one with the backing list/map wired up.
//
...
type dnsCache struct {
	...
	// entries is an LRU list ordered oldest-first (front) to newest-last (back).
	// index provides O(1) hostname → element lookup.
	entries *list.List
...
func (c *dnsCache) get(host string) ([]string, bool) {
	...
	// Defensive copy: callers must not mutate the cached slice.
...
func (c *dnsCache) put(host string, addrs []string) {
	...
	// Defensive copy so a caller mutating the slice later can't race the
	// cache value.
