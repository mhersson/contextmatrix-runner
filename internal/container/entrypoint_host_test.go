...
// overwriteDocsValidatorSnippet mirrors the case-based CM_KB_OVERWRITE_DOCS
// validator in docker/entrypoint.sh. It exits 0 on accept, 1 on reject. The
// allowlist permits a comma-separated list of doc filenames using only
// [A-Za-z0-9._,-] characters; an empty value is permitted (the default refresh
// run has no overwrite_docs). Leading dashes are rejected to defend against
// option injection if the value is later used as a CLI argument.
const overwriteDocsValidatorSnippet = `
...
// TestEntrypointOverwriteDocsValidator verifies the case-based
// CM_KB_OVERWRITE_DOCS validator rejects shell-injection payloads while
// allowing the legitimate comma-separated list of KB doc filenames the
// runner produces (manager.go:545).
func TestEntrypointOverwriteDocsValidator(t *testing.T) {
	cases := []struct {
		name      string
		value     string
		wantAllow bool
		...
		{"empty allowed", "", true},
		{"single doc", "architecture.md", true},
		{"comma-separated docs", "architecture.md,glossary.md", true},
		{"all four canonical docs", "architecture.md,code-structure.md,api-documentation.md,glossary.md", true},
		{"underscore allowed", "release_notes.md", true},
		{"semicolon rejected", "foo;rm -rf /", false},
		{"whitespace rejected", "foo bar", false},
		{"slash rejected", "../etc/passwd", false},
		{"newline rejected", "foo.md\nbar.md", false},
		{"carriage return rejected", "foo.md\rbar.md", false},
		{"NUL rejected", "foo.md\x00", false},
		{"dollar rejected", "$(whoami)", false},
		{"backtick rejected", "`whoami`", false},
		{"pipe rejected", "foo|bar", false},
		{"ampersand rejected", "foo&bar", false},
		{"leading dash rejected", "-foo.md", false},
	}

	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			cmd := exec.CommandContext(context.Background(), "sh", "-c", overwriteDocsValidatorSnippet)

			cmd.Env = append(os.Environ(), "CM_KB_OVERWRITE_DOCS="+tc.value)

			err := cmd.Run()
			if tc.wantAllow {
				assert.NoError(t, err, "value %q should be allowed", tc.value)
			} else {
				assert.Error(t, err, "value %q should be rejected", tc.value)
			}
		})
	}
}

// TestEntrypointOverwriteDocsValidatorInSource verifies the case-pattern
// string is present verbatim in docker/entrypoint.sh so refactors cannot
// silently drop the defence-in-depth check. The webhook validator already
// gates this on the API path, but if a future caller sets CM_KB_OVERWRITE_DOCS
// directly (skipping the webhook), the entrypoint must still reject bad
// values before they reach the prompt interpolation.
func TestEntrypointOverwriteDocsValidatorInSource(t *testing.T) {
	...
	// The whole-string case pattern allowing only [A-Za-z0-9._,-]. The leading
	...
	// message so operators can tell which env var failed.
	assert.Contains(t, src, "CM_KB_OVERWRITE_DOCS",
		"entrypoint.sh must mention CM_KB_OVERWRITE_DOCS in its validator error path")
}
